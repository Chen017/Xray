...
- 智能安全探针：
  - 启动面板即自动高并发探测当前服务器 IP 的 GFW 连通性状态。
  - 内置严苛的 SNI 安全自检：采用底层的 `curl` 并发模拟真实浏览器，严格校验伪装域名的证书受信度及 TLS 1.3 握手能力。直观地在面板提供 `✓` 或 `✗` 的可视化标记与自动异常预警。
...
- 便捷配置交互：
  ...
  - 内置极简美观的核心信息聚合面板，智能高亮关键参数，并自动过滤系统零碎的 UDP 临时端口。
- 日常运维支持：
  - 一键合并双端配置文件，提供带语法高亮的全量服务端 JSON 配置预览（支持 `less` 滚动）。
  - 支持查看综合日志（自动混合流式输出 access.log 与 error.log）、修改日志等级、一键测试运行。
  - 防火墙双栈端口一键统管（放行/关闭）、核心及脚本的无缝在线升级。
